...
from models import _netG, weights_init
...
#     """Generator model"""
#     def __init__(self, ngpu, dropoutG = None):
#         super(_netG, self).__init__()
#         self.ngpu = ngpu
#         if dropoutG is not None:
#             self.main = nn.Sequential(
#                 # input is Z, going into a convolution
#                 nn.ConvTranspose2d(NZ, NGF * 8, 4, 1, 0, bias=False),
#                 nn.Dropout2d(p=dropoutG, inplace = True), # Comment out to skip dropout
...
class DCGAN(object):
    ...
    def __init__(self, netG, zvector=None, batchSize=1, imageSize=64, nz=100, ngf=64, cuda=None, ngpu=None,
                 outf="/output", dropoutG = False):
        """
        DCGAN - netG Builder
    ...
    def build_model(self):
        ...
        self._model = _netG(self._ngpu, dropout=self.dropoutG, nz = self._nz, ngf = self._ngf)
    ...
    def set_G_eval(self): self._model.eval()

    # Generate the image and store in the output folder
    def generate(self, img_name = 'generated.png'):
        # print (self._input) # batchSize x nz x 1 x 1
        fake = self._model(self._input)
        vutils.save_image(fake.data, os.path.join(self._outf, img_name), normalize=True)